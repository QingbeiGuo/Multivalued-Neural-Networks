		...
		for module in model.modules():
		    if isinstance(module, nn.Conv2d):
		        ...
		        for i in range(n_partition):
		            ...
		            if self.r_partitions[i_conv_layer][i][0] + self.tau * centroids_delta[i] + abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/self.contractioncount < self.r_partitions[i_conv_layer][i][1] + self.tau * centroids_delta[i] - abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/self.contractioncount:
		                r_partitions.append([self.r_partitions[i_conv_layer][i][0] + self.tau * centroids_delta[i] + abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/self.contractioncount, self.r_partitions[i_conv_layer][i][1] + self.tau * centroids_delta[i] - abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/self.contractioncount])
		            else:
		                #r_partitions.append([(self.r_partitions[i_conv_layer][i][0] + self.r_partitions[i_conv_layer][i][1])/2, (self.r_partitions[i_conv_layer][i][0] + self.r_partitions[i_conv_layer][i][1])/2])
		                #r_partitions.append([centroids[i], centroids[i]])
