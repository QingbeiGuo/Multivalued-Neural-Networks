...
class TrainingFineTuner_LeNet:
	def __init__(self, train_path, test_path, model):
		...
		self.d_partitions = []; #[[...],[...],[...],[...]] contraction delta
		self.centroids = [];    #[1,2,3,4] save mean before
		self.centroids_ = [];   #[1,2,3,4] save mean after
		self.contractioncount  = 500; #the number of partition contraction batch_count * epoch_count
...
##################################################################################################
	def train(self, optimizer = None, epoches = -1, batches = -1):
        #resume
		if os.path.isfile("list_epoches"):
		    list_epoches = torch.load("list_epoches")
		    print("list_epoches resume:", list_epoches)
		    self.n_partitions = torch.load("n_partitions")
		    print("n_partitions resume:", self.n_partitions)
		    self.c_w = torch.load("c_w")
		    print("c_w resume:", self.c_w)
		    self.r_partitions = torch.load("r_partitions")
		    print("r_partitions resume:", self.r_partitions)
		    self.w_partitions = torch.load("w_partitions")
		    print("w_partitions resume:", self.w_partitions)
		    self.i_partitions = torch.load("i_partitions")
		    print("i_partitions resume:", self.i_partitions)
		    self.d_partitions = torch.load("d_partitions")
		    print("d_partitions resume:", self.d_partitions)
		    self.centroids = torch.load("centroids")
		    print("centroids resume:", self.centroids)
		    self.centroids_ = torch.load("centroids_")
		    print("centroids_ resume:", self.centroids_)
		    self.contractioncount = torch.load("contractioncount")
		    print("contractioncount resume:", self.contractioncount)
		    self.model = torch.load("model")
		    print("model resume:", self.model)
		    self.accuracys1 = torch.load("accuracys1")
		    print("accuracys1 resume:", self.accuracys1)
		    self.accuracys5 = torch.load("accuracys5")
		    print("accuracys5 resume:", self.accuracys5)
		else:
		...
		for i in list_epoches[:]:
		    ...
		    if i == self.startquantepoch:
		        ...
		        fine_tuner.calculate_contractiondelta()
		    ...
		    if optimizer is None:
		        optimizer = \
                    optim.SGD(self.model.parameters(), lr=self.learningrate, momentum=0.9, weight_decay=0.0001)
		    ...
		    if (i >= self.startquantepoch) & (i <= self.startquantepoch + self.contractionepoch + 1):
            ...
            #save
		    if i == self.startquantepoch + self.contractionepoch - 5:
		        torch.save(list_, "list_epoches" + str(i))
		        torch.save(self.n_partitions, "n_partitions" + str(i))
		        torch.save(self.c_w, "c_w" + str(i))
		        torch.save(self.r_partitions, "r_partitions" + str(i))
		        torch.save(self.w_partitions, "w_partitions" + str(i))
		        torch.save(self.i_partitions, "i_partitions" + str(i))
		        torch.save(self.d_partitions, "d_partitions" + str(i))
		        torch.save(self.centroids, "centroids" + str(i))
		        torch.save(self.centroids_, "centroids_" + str(i))
		        torch.save(self.contractioncount, "contractioncount" + str(i))
		        torch.save(self.accuracys1, "accuracys1" + str(i))
		        torch.save(self.accuracys5, "accuracys5" + str(i))

		    torch.save(list_, "list_epoches")
		    torch.save(self.n_partitions, "n_partitions")
		    torch.save(self.c_w, "c_w")
		    torch.save(self.r_partitions, "r_partitions")
		    torch.save(self.w_partitions, "w_partitions")
		    torch.save(self.i_partitions, "i_partitions")
		    torch.save(self.d_partitions, "d_partitions")
		    torch.save(self.centroids, "centroids")
		    torch.save(self.centroids_, "centroids_")
		    torch.save(self.contractioncount, "contractioncount")
		    torch.save(self.model, "model")
		    torch.save(self.model, "model" + str(i))
		    torch.save(self.accuracys1, "accuracys1")
		    torch.save(self.accuracys5, "accuracys5")
...
##################################################################################################
	def quantization_partitions(self): #the quantization of weights
		i_conv_layer = 0
        #for the conv_layer weights
		for module in self.model.modules():
		    if isinstance(module, nn.Conv2d):
...
##################################################################################################
	def update_partitions(self, epoch): #update the weights, centroids and edge
		i_conv_layer = 0
        #for the conv_layer weights
		for module in self.model.modules():
		    if isinstance(module, nn.Conv2d):
		        ...
		        epoch = min(self.contractionepoch - 1, epoch)
		        r_partitions = []
		        for i in range(n_partition):
		            ...
		            if (self.r_partitions[i_conv_layer][i][0] != self.r_partitions[i_conv_layer][i][1]) and (self.r_partitions[i_conv_layer][i][0] + self.tau * centroids_delta[i] + self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]) < self.r_partitions[i_conv_layer][i][1] + self.tau * centroids_delta[i] - self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0])):
		                r_partitions.append([self.r_partitions[i_conv_layer][i][0] + self.tau * centroids_delta[i] +  self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]), self.r_partitions[i_conv_layer][i][1] + self.tau * centroids_delta[i] - self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0])])
		                print("partition delta", self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]), "-", self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]), "-", self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][0] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]) + self.d_partitions[i_conv_layer][i][epoch] * abs(self.r_partitions[i_conv_layer][i][1] - self.centroids[i_conv_layer][i])/abs(self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]))
		            else:
		                #r_partitions.append([(self.r_partitions[i_conv_layer][i][0] + self.r_partitions[i_conv_layer][i][1])/2, (self.r_partitions[i_conv_layer][i][0] + self.r_partitions[i_conv_layer][i][1])/2])
		                #r_partitions.append([centroids[i], centroids[i]])
...
##################################################################################################
	def calculate_contractiondelta(self):
		start_cd = 1;
		fin_cd = 0.001;

		cd_decay = (fin_cd/start_cd)**(1./self.contractionepoch)

		cd = start_cd
		cd_sum = 0
		cd_list = [0]*self.contractionepoch
		for i in range(self.contractionepoch):
		    cd = cd * cd_decay
		    cd_sum = cd_sum + cd
		    cd_list[i] = cd

		for i in range(self.contractionepoch):
		    cd_list[i] = cd_list[i] / cd_sum
		...
		for module in self.model.modules():
		    if isinstance(module, nn.Conv2d):
		        ...
		        if len(self.i_partitions) < i_conv_layer+1:
		            self.d_partitions.append([])
		        else:
		            self.d_partitions[i_conv_layer] = []

		        for i in range(n_partition):
		            if len(self.d_partitions[i_conv_layer]) < i+1:
		                self.d_partitions[i_conv_layer].append([])

		            p_len = self.r_partitions[i_conv_layer][i][1] - self.r_partitions[i_conv_layer][i][0]
		            self.d_partitions[i_conv_layer][i] = list(np.array(cd_list) * p_len)
		            print("self.d_partitions: ", i_conv_layer, "-", i, "-", p_len, "-", self.d_partitions[i_conv_layer][i])
...
if __name__ == '__main__':
    ...
    #compress the deep model
		...
		fine_tuner.train(epoches = 1000, batches = -1)
